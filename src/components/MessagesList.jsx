...
export const MessagesList = () => {
  ...
  useEffect(() => {
    if (container.current && messages.length > 0) {
  ...
  return (
    ...
    >
      ...
      {messages.map((message, index) => {
        if (!message || !message.answer) return null;
        
        const isCurrentMessage = currentMessage === index;
        ...
        return (
          <div key={`message-${index}`}>
            ...
            {message.answer.grammarBreakdown && message.answer.grammarBreakdown.length > 0 && (
              <div className="p-5 mt-5  bg-gradient-to-br from-pink-200/20 to-pink-500/20 rounded-xl">
                ...
                {message.answer.grammarBreakdown.map((grammar, i) => (
                  <div key={`grammar-${index}-${i}`} className="mt-3">
                    ...
                    {grammar.chunks && (
                      <div className="mt-3 flex flex-wrap gap-3 items-end">
                        {grammar.chunks.map((chunk, chunkIndex) => (
                          <div key={`chunk-${index}-${i}-${chunkIndex}`} className="p-2 bg-black/30 rounded-md">
                            {chunk.japanese && <div className="text-white/90 text-4xl font-jp">{renderJapanese(chunk.japanese)}</div>}
